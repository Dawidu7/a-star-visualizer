...
class AStar:
  ...
  def find_path(self, start_tile: Tile, end_tile: Tile, draw: Callable[[], None]) -> list[Tile]:
    count = 0
    ...
    while not open_queue.empty():
      ...
      if current_tile == end_tile:
        return self._reconstruct_path(came_from, current_tile, draw)
      ...
      for neighbour in neighbours:
        ...
        if g < g_score.get(neighbour, float("inf")):
          ...
          if not neighbour in open_hash:
            ...
            neighbour.type = TileType.OPEN

      draw()

      if current_tile != start_tile:
        current_tile.type = TileType.CLOSED
  ...
  def _reconstruct_path(self, came_from: dict[Tile, Tile], current_tile: Tile, draw: Callable[[], None]) -> list[Tile]:
    path = [current_tile]

    while current_tile in came_from:
      current_tile = came_from[current_tile]
      path.append(current_tile)
      current_tile.type = TileType.PATH
      draw()
